...
(function () {
  ...
  function renderPrintMeta(n) {
    ...
    // 「收录 N 项」而不是「共 N 项」：竖版打印这条路上，纸上也只有这一行在解释那个数字
    bits.push('收录 ' + n + ' 项（非各校全部专业）');
  ...
  var MANUAL = {
    uk: [
      // 放在最前：读表之前得先知道这张表不完整，否则「牛津 25 项」会被读成「牛津一共 25 个专业」。
      // 与打印册封面、Word 封面、静态页的「收录 N 个」是同一件事的四处说法（见 sheetHTML 注释）
      { t: '收录范围（先读这条）', items: [
        '**本站是选编**：收录各校在 10 个热门学科方向（生化医药 / 物理 / 工程 / 经管金融 / 计算机数据 / 统计数学 / 教育 / 人文语言 / 社科法律 / 传媒）的本科课程，**不是各校完整的专业目录**——没有收录，不等于该校没有这个专业。',
        '**表内的「N 项」是本站收录条数，不是该校的专业总数。** 收录以各方向的主要课程为主，完整课程目录请以各校官网为准。',
        '每一行「操作」列里都有该专业的官网课程页链接，可逐条核对。'
      ]},
      { t: '成绩口径', items: [
    ...
    hk: [
      // 与英国板块同一段（理由见那边的注释）：读数之前先知道这张表不完整
      { t: '收录范围（先读这条）', items: [
        '**本站是选编**：收录各校在 10 个热门学科方向（生化医药 / 物理 / 工程 / 经管金融 / 计算机数据 / 统计数学 / 教育 / 人文语言 / 社科法律 / 传媒）的本科课程，**不是各校完整的专业目录**——没有收录，不等于该校没有这个专业。',
        '**表内的「N 项」是本站收录条数，不是该校的专业总数。** 八校实际开设的本科课程远多于此，完整课程目录请以各校官网为准。',
        '每一行「操作」列里都有该专业的官网课程页链接，可逐条核对。'
      ]},
